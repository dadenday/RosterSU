...
def fuzzy_match_markers(
    ...
    for marker in markers:
        ...
        # Exact substring match (fast path - handles 99% of cases)
        if marker_upper in text_upper:
            count += 1
            continue
        # Fuzzy fallback using stdlib SequenceMatcher
        # partial_ratio equivalent: check if marker is substring of any text window
        ratio = SequenceMatcher(None, text_upper, marker_upper).ratio() * 100
        if ratio >= threshold:
            count += 1
    return count
...
def parse_file(file_stream, filename, alias_regex):
    ...
    try:
        if filename.endswith(".csv"):
            ...
            # Use openpyxl for Excel file processing (pure Python, no compilation required)
            t0 = time.monotonic()
            wb_openpyxl = openpyxl.load_workbook(
            ...
            # Read all sheets into memory using openpyxl
            sheets_data = []
            for sheet_name in wb_openpyxl.sheetnames:
                ws = wb_openpyxl[sheet_name]
                rows = [list(row) for row in ws.iter_rows(values_only=True)]
            ...
            # Extract merged ranges for zone detection (already have workbook open)
            zone_blocks_map = {}
            try:
                for sheet_name in wb_openpyxl.sheetnames:
                    ws = wb_openpyxl[sheet_name]
            ...
            # Close workbook after reading all data and zone info
            wb_openpyxl.close()
