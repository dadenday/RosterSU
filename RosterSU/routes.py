...
from roster_single_user import (
    rt, serve, debug_log, layout, get_config, save_config, get_aliases,
    get_aircraft_config, compile_alias_regex, parse_file, ParseContext,
    consolidate_file_results, save_entries_bulk, log_debug,
)
from config import (
    DB_FILE, EXPORT_DIR, AUTO_INGEST_DIR, INGEST_INTERVAL, MAX_UPLOAD_MB,
    SAFE_THRESHOLD,
)
from state import try_get_app_status, get_app_status, update_status, APP, bump_db_rev, SHUTDOWN_EVENT
...
from components import (
    format_date_vn, format_shift_display,
    shift_color, shift_text_class, build_copy_text,
    RosterCard, sort_flights_by_open_time,
    is_flight_card_active, RosterList, _init_components,
...
async def post_upload(file: UploadFile):
    ...
    if len(content) > MAX_UPLOAD_MB * 1024 * 1024:
        ...
        return Span(f"File too large (>{MAX_UPLOAD_MB}MB)", cls="st-run", id="status-indicator")
    ...
    content = Div(
        ...
        Form(
            ...
            Div(
                ...
                Textarea(aliases_val, name="aliases", rows=2, 
            ...
            Div(
                H5("📖 Xem lịch tĩnh"),
                P("Tạo file HTML tĩnh để xem nhanh qua Termux Widget. Không cần mở app.",
                  style="font-size:0.8rem; color:var(--muted); margin-bottom:0.5rem;"),
                # Scope radio buttons
                Div(
                    ...
                    # Radio options
                    Div(
                        Input(type="radio", id="scope_all", name="static_html_scope", value="all",
                              **({"checked": ""} if config.get("static_html_scope") == "all" else {})),
                        Label("Tất cả", for_="scope_all", style="margin-right:0.75rem;"),

                        Input(type="radio", id="scope_month", name="static_html_scope", value="current_month",
                              **({"checked": ""} if config.get("static_html_scope") in ("current_month", None) else {})),
                        Label("Tháng này", for_="scope_month", style="margin-right:0.75rem;"),

                        Input(type="radio", id="scope_latest", name="static_html_scope", value="latest",
                              **({"checked": ""} if config.get("static_html_scope") == "latest" else {})),
                        Label("Mới nhất", for_="scope_latest", style="margin-right:0.75rem;"),

                        Input(type="radio", id="scope_n", name="static_html_scope", value="latest_n",
                              **({"checked": ""} if config.get("static_html_scope") == "latest_n" else {})),
                        Label("N mục mới nhất:", for_="scope_n"),
                        Input(type="number", id="scope_count", name="static_html_count", value=str(config.get("static_html_count", 5)),
                              min="1", max="100", style="width:4rem; margin-left:0.3rem;"),
        ...
        Div(
            ...
            P("Nhấn nút để tạo file HTML từ cài đặt đã lưu.",
              style="font-size:0.8rem; color:var(--muted); margin-bottom:0.5rem;"),
            Button("🔄 Tạo file HTML",
                   hx_post="/export/html",
                   hx_target="#static-viewer-status",
                   cls="btn-act",
                   style="width:100%;"),
            Div(id="static-viewer-status"),
            P("File: schedule.html (mở nhanh qua Termux Widget)",
...
def post_export_html():
    ...
    try:
        ...
        if result["success"]:
            return Div(
                P(f"✅ Đã tạo: {result['entry_count']} mục → schedule.html", cls="text-green-700"),
                id="static-viewer-status"
            )
        else:
            return Div(
                P(f"❌ Lỗi: {html.escape(str(result['error']), quote=True)}", cls="text-red-600"),
                id="static-viewer-status"
            )
    except Exception as e:
        return Div(
